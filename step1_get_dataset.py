...
print("Please be patient while dataset is being prepared.")

log.info("Building data directory")
bs_data_dir = os.environ.get('BRAINSTORM_DATA_DIR', 'data')
if not os.path.exists(bs_data_dir):
    log.debug("Creating dir: " + bs_data_dir)
...
log.info("Using data directory: " + bs_data_dir)
if not os.path.exists(mnist_file):
    log.info("Downloading MNIST data ...")
    urlretrieve(url, mnist_file)
    print("Retrieved MNIST dataset.")
else:
    print("MNIST dataset already existed in ", bs_data_dir)

log.debug("Extracting MNIST data ...")
with gzip.open(mnist_file, 'rb') as f:
    if sys.version_info < (3,):
        ds = pickle.load(f)
    else:
        ds = pickle.load(f, encoding='latin1')
    log.info("Unpacked MNIST data file.")
...
log.debug("Creating HDF5 dataset ...")
